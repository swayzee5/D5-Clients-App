...
type Step = "grid" | "confirm" | "checkin" | "whatsapp" | "done";
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden border transition-all ${
      ...
      <div className="flex items-center justify-between px-3 pt-3 pb-2">
        <span className="px-2.5 py-0.5 border-2 border-d5-gold text-d5-gold text-xs font-bold rounded-full">
          {seriesLabel}
        </span>
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex gap-4">
          {exercise.reps && (
            <div><p className="text-gray-500 text-xs">Reps</p><p className="text-white font-semibold text-sm">{exercise.reps}</p></div>
          )}
          {exercise.rest_seconds && (
            <div><p className="text-gray-500 text-xs">Récup.</p><p className="text-white font-semibold text-sm">{formatRest(exercise.rest_seconds)}</p></div>
          )}
        </div>
    ...
    return (
      <div className="card space-y-4 mt-4">
        <div className="text-center space-y-1">
          ...
          <p className="text-d5-muted text-xs">Copie ce message et colle-le dans le groupe WhatsApp</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {exercises.map((ex, i) => (
          <ExerciseCard key={ex.id} exercise={ex} index={i}
            checked={checked.has(ex.id)} onCheck={() => toggle(ex.id)} />
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm space-y-5">
            ...
            <div className="space-y-3">
              <button onClick={triggerComplete}
                className="w-full py-3.5 bg-d5-gold text-black font-bold rounded-xl text-sm">
                Confirmer quand même
              </button>
              <button onClick={() => { setChecked(new Set(exercises.map((e) => e.id))); setShowModal(false); triggerComplete(); }}
                className="w-full py-3.5 border-2 border-d5-gold text-d5-gold font-bold rounded-xl text-sm">
