      ...
      <div>
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <span className="text-d5-muted text-xs">
            {MUSCLE_LABELS[rebootSession.muscle_group] ?? rebootSession.muscle_group}
          </span>
        </div>
        <h1 className="text-xl font-bold text-white">{rebootSession.name}</h1>
        {rebootSession.description && (
          <p className="text-d5-muted text-sm mt-1">{rebootSession.description}</p>
        )}
        <div className="flex items-center gap-3 mt-2 text-xs text-d5-muted">
          <span>{exercises.length} exercices</span>
          {rebootSession.duration_minutes && (
            <span>⏱ {rebootSession.duration_minutes} min</span>
          )}
        </div>
      </div>
            ...
            <div key={ex.id} className="card">
              {ex.vimeo_video_id && (
                <div className="mb-3 rounded-xl overflow-hidden aspect-video bg-black">
                  <iframe
                    src={`https://player.vimeo.com/video/${ex.vimeo_video_id}?badge=0&autopause=0&player_id=0`}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    {ex.sets && ex.reps && (
                      <span className="text-d5-gold text-xs font-semibold">
                        {ex.sets} × {ex.reps}
                      </span>
                    )}
                    {ex.rest_seconds && (
                  ...
                  {ex.notes && (
                    <p className="text-d5-muted text-xs mt-1 italic">{ex.notes}</p>
                  )}
                </div>
              </div>
            </div>
