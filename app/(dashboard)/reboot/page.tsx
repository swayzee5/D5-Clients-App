...
const TIPS = [
  { emoji: "🔥", title: "La régularité avant l’intensité", body: "3 séances bien faites valent mieux que 5 séances bâclées. Avance à ton rythme." },
  { emoji: "💧", title: "Hydratation", body: "2L d’eau minimum par jour. Ton corps a besoin de carburant pour récupérer." },
  { emoji: "😴", title: "Le sommeil, ton meilleur allié", body: "7–8h de sommeil accélèrent ta progression. La récupération se fait la nuit." },
  { emoji: "🥗", title: "Protéines à chaque repas", body: "Vise 1.6g de protéines par kg de poids de corps. Ça préserve le muscle." },
];
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-d5-gold/20 to-transparent border border-d5-gold/30 rounded-2xl p-5">
        ...
        <p className="text-gray-400 text-sm mt-1">3 groupes musculaires. Salle ou maison. Toi qui choisis.</p>
        <div className="mt-4">
          <div className="flex items-center justify-between text-xs mb-1.5">
            <span className="text-gray-400">{completedCount}/{totalSessions} séances complétées</span>
          ...
          return (
            <div key={muscle}>
              <div className="flex items-center gap-2 mb-2.5">
                ...
                <h3 className="text-white font-medium text-sm">{MUSCLE_LABELS[muscle]}</h3>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {list.map((s) => (
                  <Link key={s.id} href={`/reboot/${s.id}`}>
                    <div className={`card h-full transition-all ${
                      s.completed ? "border-d5-gold/30 bg-d5-gold/5" : "hover:border-white/20"
                    }`}>
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {s.completed && <CheckCircle2 size={15} className="text-d5-gold shrink-0" />}
                      </div>
                      <p className={`text-sm font-medium leading-snug ${s.completed ? "text-d5-gold" : "text-white"}`}>
                        {s.name}
                      </p>
                      <div className="flex items-center gap-1.5 mt-2">
                        <Dumbbell size={11} className="text-d5-muted" />
                        <span className="text-d5-muted text-xs">{s.exercise_count} exercices</span>
                      </div>
                      {s.duration_minutes && <p className="text-d5-muted text-xs mt-0.5">⏱ {s.duration_minutes} min</p>}
                    </div>
                  </Link>
                ))}
              </div>
      ...
      <div className="card border-d5-gold/20 bg-gradient-to-br from-d5-gold/5 to-transparent">
        <p className="text-xs text-d5-gold font-semibold uppercase tracking-wider mb-2">Après le challenge</p>
        <p className="text-white font-semibold text-sm">Prêt pour la suite ?</p>
        <p className="text-d5-muted text-xs mt-1">Le coaching individuel D5 va plus loin : programme personnalisé, nutrition sur-mesure, suivi hebdomadaire.</p>
      </div>
    </div>
  );
}
