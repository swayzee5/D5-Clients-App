...
export async function addProgressEntry(formData: FormData) {
  ...
  const keys = ["entryDate", "heightCm", "weightKg", "waistCm", "chestCm", "hipsCm", "armsCm", "thighsCm", "notes"]
  const raw = Object.fromEntries(keys.map((k) => [k, formData.get(k) || undefined]))
  ...
  await pool.query(
    `INSERT INTO progress_entries
       (client_id, entry_date, height_cm, weight_kg, waist_cm, chest_cm, hips_cm, arms_cm, thighs_cm, notes)
    ...
    [session.user.id, d.entryDate, d.heightCm ?? null, d.weightKg ?? null, d.waistCm ?? null,
     d.chestCm ?? null, d.hipsCm ?? null, d.armsCm ?? null, d.thighsCm ?? null, d.notes ?? null]
  )
