...
export default async function ProgrammePage() {
          ...
          {isRebootOnly ? (
            <>
              ...
              <p className="text-d5-muted text-sm mt-1 max-w-xs">
                Les programmes personnalisés font partie du suivi coaching individuel. Contacte ton coach pour en savoir plus.
              </p>
            </>
          ) : (
            <>
  ...
  const totalExercises = activeProgram.sessions.reduce((acc, s) => acc + s.exercises.length, 0);
      ...
      <div className="card">
        ...
        {activeProgram.start_date && (
          <p className="text-d5-muted text-xs mt-2">
            Début :{" "}
            {new Intl.DateTimeFormat("fr-FR", { day: "numeric", month: "long", year: "numeric" }).format(new Date(activeProgram.start_date))}
          </p>
        )}
        <div className="flex gap-6 mt-4 pt-4 border-t border-d5-border">
          <div>
            ...
            <p className="text-d5-muted text-xs mt-0.5">Séance{activeProgram.sessions.length !== 1 ? "s" : ""}</p>
          </div>
          <div>
        ...
        {activeProgram.sessions.map((sess, i) => (
          <div key={sess.id} className="card">
            ...
            {sess.exercises.length === 0 ? (
              <p className="text-d5-muted text-sm text-center py-3">Aucun exercice</p>
            ) : (
              <div>
                <div className="grid grid-cols-12 gap-2 pb-2 border-b border-d5-border">
                  <div className="col-span-6 text-xs text-d5-muted uppercase tracking-wider">Exercice</div>
                  <div className="col-span-2 text-xs text-d5-muted text-center uppercase tracking-wider">Séries</div>
                  <div className="col-span-2 text-xs text-d5-muted text-center uppercase tracking-wider">Reps</div>
                  <div className="col-span-2 text-xs text-d5-muted text-center uppercase tracking-wider">Repos</div>
                </div>
                  ...
                  {sess.exercises.map((ex) => (
                    <div key={ex.id} className="grid grid-cols-12 gap-2 py-3 items-center">
                      ...
                      <div className="col-span-2 text-center">
                        <span className="text-d5-gold font-bold">{ex.sets ?? "—"}</span>
                      </div>
                      <div className="col-span-2 text-center">
                        <span className="text-white text-sm">{ex.reps ?? "—"}</span>
                      </div>
                      <div className="col-span-2 text-center">
                        <span className="text-d5-muted text-sm">{ex.rest_seconds != null ? `${ex.rest_seconds}s` : "—"}</span>
                      </div>
                    </div>
                  ))}
                </div>
